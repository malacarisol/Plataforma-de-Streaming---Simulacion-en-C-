...
void generator(cStreaming *stream)
{
	...
	for (unsigned int i = 0;i < pais.size();i++) {
		...
		for (int i = 0;i < x;i++)
		{
			...
			stream->AgregarUsuario(new cUsuario(edades[j], username[j], passwords[j], false, paises[j], apellidos[j], nombres[j], type));
		}
	...
	string namemovies = "Mad Max-Fury-Titanic-Maze Runner-Hunger Games-Jojo Rabbit-Hangover-21 Blackjack-Harry Potter-Zatura-El origen-Pets-Toy Story-The Lion King-the notebook-High school musical-Entrenando a mi dragon- cars-cars2-Lilo y stich- Hannah Montana-El conjuro.";
	
	for (int i = 0;i < nombre.length();i++) {
		if (namemovies[i] != '-') {
			aux.push_back(namemovies[i]);
		}
		else {
	...
	for ( i = 0;i < juegos.size();i++)
	{
		...
		tipoServicio type1 = static_cast<tipoServicio>(0);            //el nivel por deafult empieza en 1, luego el programa aumenta a medida que progresa el jugador
		stream->AgregarService(new cJuegos(1,type,juegos[i],b,type1, paisesprohibidosJ));        //Juegos no necesita ser random, se actualiza en la lista de servicios a medida que se agregan juegos a  la lista
	}
	...
	for (a = i ;a < Musica.size();a++)
	{
		...
		stream->AgregarService(new cMusica(l, type, Musica[a], b, type2, paisesprohibidosM));
	}
	...
	for (b=a;b < peliculas.size() ;b++)
	{
		...
		stream->AgregarService(new cPeliculas(c, type, peliculas[i], b, type3, paisesprohibidosP));
	}
}
